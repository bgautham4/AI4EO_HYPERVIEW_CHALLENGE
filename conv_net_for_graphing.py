...
test=ConvNet(channels=50,kernel_size=5,stride_len=5,hidden_full_layer=True,size_hidden_layer=2000)
loss=nn.MSELoss()
num_epochs=100
mini_batch_size=50
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        Outs=test(x_train_graphing)
        L=loss(Outs,y_train_graphing)
        Outs2=test(x_test_graphing)
        L2=loss(Outs2,y_test_gr_tensor)
        Loss.append(L)
        Loss2.append(L2)
    optimizer.zero_grad()
    
plt.plot(range(1,num_epochs+1),Loss,linestyle='dotted',color='b',label='train_data')
plt.plot(range(1,num_epochs+1),Loss2,color='r',label='test_data',linewidth=2.5)
plt.xlabel('epoch')
plt.ylabel('Loss')
plt.legend()
plt.savefig('conv_net_loss.eps', bbox_inches='tight',pad_inches = 0)
plt.show()
